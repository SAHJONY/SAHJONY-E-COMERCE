...
export default function OwnerPage() {
  ...
  async function requestSupplierEvidence(id:string){
    setBusyAction(`supplier-${id}`); setError('');
    try {
      const r=await fetch('/api/owner/sourcing',{method:'PATCH',headers:{'content-type':'application/json','x-owner-token':token},body:JSON.stringify({supplierId:id,action:'request_supplier_evidence'})});
      if(!r.ok) throw new Error('supplier'); await loadOwnerData(token);
